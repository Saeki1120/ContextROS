...
def gen_func_def(ast):
    ...
    mojiretu = ast[0] + '(' + ast[1] + ')'

    return mojiretu

def gen_if(ast, layer):
	l = []
	for i, kase in enumerate(ast):
		print kase
		line1 = gen_func_def(kase)
		line2 = "if (cros.now_layer() == " + '"' + layer + '"' + ") {return " + kase[2] + '(' + bind_comma(kase[3]) + ");}\n"
		#print line1
		#print line2
		l.append([kase[0], line1, "CROS cros;\n", line2])
	#print l
	return l


def gen_elif(ast, katamari, num, layer):
	for i, kase in enumerate(ast):
		#print kase
		if kase[0] == katamari[i][0]:
			sen = "else if (cros.now_layer() == " + '"' + layer + '"' + "){return " \
...
def gen(ast):
	...
	for i, kase in enumerate(l):
		if i == 0:
			katamari = gen_if(kase, ast[1][1])
		else:
			gen_elif(kase, katamari, i, ast[i+1][1])
